#!/bin/bash
#
# Q4 SNR Sweep Runner — v4 Channel Proxy Edition
# ================================================
# Uses v4.py (Unified ChannelProducer baseline, no v6/v7 physics fixes,
# no attach-stable). For pure performance/throughput baselining vs v7/v8.
#
# Runs launch_all_v4.sh repeatedly across a set of input-SNR points,
# collecting at least MAX_FRAMES frames per point, and organizes the
# outputs under a single sweep root directory.
#
# Usage:
#   sudo bash run_q4_snr_sweep_v4.sh [options via env vars]
#
# Env vars:
...
#   PROXY_VER        Proxy version                 (default: v4)
#   NUM_UES          Number of UEs                 (default: 1)
#   UE_SPEED         UE speed in m/s               (default: empty, v4 internal)
#   GT_SAVE_EVERY    Save GT every N UL slots      (default: 10)
#
# v4에서 제거된 환경변수 (v6/v7 전용):
#   DEFAULT_DIST_M, ATTACH_STABLE_*, P1B_DISTANCE_MODE
#
# Ctrl+C behavior:
...
export MIN_SRS_BINS GT_SAVE_EVERY  # inherited by launch_all_v4.sh

# ── Paths ────────────────────────────────────────────────────────────────
SCRIPT_DIR="$(cd "$(dirname "${BASH_SOURCE[0]}")" && pwd)"
PROJ_DIR="/home/dclcom61/OAI_luuuuuu/DevChannelProxyJIN"
LAUNCH_SCRIPT="${SCRIPT_DIR}/launch_all_v4.sh"
...
# ── Banner ───────────────────────────────────────────────────────────────
echo "========================================================================"
echo "  Q4 SNR Sweep — v4 Channel Proxy (baseline, no attach-stable)"
echo "========================================================================"
...
echo "  GT save_every : ${GT_SAVE_EVERY}"
...
if [ ! -f "$MANIFEST" ]; then
    {
        ...
        echo "# gt_save_every: ${GT_SAVE_EVERY}"
        echo "# ue_speed: ${UE_SPEED:-default}"
        echo "# ----------------------------------------"
...
for SNR in $SNR_POINTS; do
    ...
    bash "$LAUNCH_SCRIPT" \
        ...
        -snr "$SNR" \
        -mf  "$MAX_FRAMES" \
        $P1B_FLAG $RX_FLAG $AGC_FLAG $SEED_FLAG $SPEED_FLAG &
    LAUNCH_PID=$!
