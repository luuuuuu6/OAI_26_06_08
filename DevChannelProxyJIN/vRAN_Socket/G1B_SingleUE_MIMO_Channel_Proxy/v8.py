"""
================================================================================
v8.py - GPU IPC V6 + MIMO CUDA Graph Pipeline + NOISE_PREP Optimized (G1B)

[v7 → v8 변경사항]
  1. NoiseProducer: 별도 스레드에서 AWGN 난수를 batch 사전 생성 (RingBuffer)
  2. _regenerate_noise(): randn() → NoiseRingBuffer view+release (memcpy only)
  3. noise OFF 시 NoiseProducer 미생성 (하위 호환)
...
- NOISE_PREP 최적화: NoiseProducer batch 사전 생성, pipeline은 memcpy만
================================================================================
"""
...
class Proxy:
    ...
    def _init_channel(self, buffer_len, buffer_symbol_size):
        ...
        self._noise_buffers_dl = None
        self._noise_buffers_ul = None
        ...
        if noise_enabled and GPU_AVAILABLE:
            ...
            self._noise_buffers_dl = RingBuffer(
                shape=(2, noise_len_dl), dtype=cp.float64, maxlen=256)
            np_dl = NoiseProducer(self._noise_buffers_dl, noise_len_dl)
            self.noise_producers.append(np_dl)
            self._noise_buffers_ul = RingBuffer(
                shape=(2, noise_len_ul), dtype=cp.float64, maxlen=256)
            np_ul = NoiseProducer(self._noise_buffers_ul, noise_len_ul)
        ...
        if not self.custom_channel:
            self.pipeline_dl = GPUSlotPipeline(
                ...
                noise_buffer=self._noise_buffers_dl, **pipeline_common)
            self.pipeline_ul = GPUSlotPipeline(
                FFT_SIZE, n_tx_in=self.ue_ant, n_rx_out=self.gnb_ant,
                noise_buffer=self._noise_buffers_ul, **pipeline_common)
    ...
    def _warmup_pipeline(self):
        ...
        if not self.custom_channel:
            print("[GPU IPC V8] Bypass mode — channel warmup skipped")
            return
        print("[GPU IPC V8] Pre-warming DL/UL pipelines (XLA compile + CUDA Graph)...")
        t0 = time.time()
        ...
        print(f"[GPU IPC V8] DL/UL pipelines ready ({time.time()-t0:.1f}s)")
...
def main():
    ...
    if args.enable_gpu and GPU_AVAILABLE:
        ...
        if args.mode == "gpu-ipc":
            print(f"  + [v8] GPU IPC V6 per-buffer antenna (gNB={args.gnb_ant}, UE={args.ue_ant})")
            print(f"  + [v8] gNB array: {args.gnb_ny}x{args.gnb_nx}, UE array: {args.ue_ny}x{args.ue_nx}")
            print("  + [v8] DL: broadcast*sum channel (CUDA Graph capturable)")
            print("  + [v8] UL: H^T broadcast*sum (DL/UL symmetric)")
            print("  + [v8] OFDM symbol: GPU index array (no Python for-loop)")
            print("  + [v8] Pre-allocated intermediate buffers (out= params)")
            print("  + [v8] CH_COPY: c128 unified, view+release (no .copy()), conditional zero-fill")
            print("  + [v8] Channel FFT pre-computed in ChannelProducer (fft(H) removed from Graph)")
            print(f"  + [v8] Noise model: {noise_mode} " +
                  (f"(SNR={snr_dB}dB)" if noise_mode == "relative" else
                   f"(floor={noise_dBFS}dBFS)" if noise_mode == "absolute" else "(off)"))
            if noise_mode != "none":
                print("  + [v8] NOISE_PREP: pre-generated (NoiseProducer batch=64, RingBuffer=256)")
