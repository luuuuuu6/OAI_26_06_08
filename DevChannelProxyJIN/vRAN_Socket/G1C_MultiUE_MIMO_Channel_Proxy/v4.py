...
class GTBatchSaver:
    ...
    def __init__(self, gt_dir, num_ues, gnb_ant, ue_ant, fft_size=2048,
        ...
        print(f"[GT Saver] Initialized: dir={gt_dir}, save_every={self.save_every}, "
    ...
    def _flush(self, ue_idx):
        ...
        print(f"[GT Saver] UE[{ue_idx}] batch seq{seq} saved "
              f"({len(slot_ids)} slots, shape={h_matrices.shape}) → {fname}")
...
class UnifiedChannelProducerProcess(_mp_ctx.Process):
    ...
    def run(self):
        ...
        if _child_seed is not None:
            ...
            print(f"[v4 ChannelProducer] RNG seed={_child_seed} "
        ...
        if _child_seed is not None:
            ...
            print(f"[v4 ChannelProducer] sionna config.seed={_child_seed}")
        ...
        def _generate_eager():
            ...
            return _tf.cast(h_norm, _tf.complex128)
    ...
    def _ipc_ul_superposition_slot(self, ts, nsamps, active_ues=None):
        ...
        for k in ues:
            ...
            try:
                ...
                print(f"[v3 WARN] UE[{k}] UL channel buffer timeout — skip this UE")
                n_bypassed += 1
                continue
            if channels.shape[0] < N_SYM:
                lib = cp if GPU_AVAILABLE else np
...
def main():
    ...
    if args.seed is not None:
        ...
        print(f"[v4] Global RNG seed set to {args.seed} "
              f"(tf + np + cp + python random)")

    global Speed
    if args.speed is not None:
        Speed = args.speed
        print(f"[v4] UE speed overridden to {Speed} m/s")
