#!/bin/bash
#
# G1C 통합 런처 — v4 Channel Proxy (Unified ChannelProducer baseline)
#
# v4 특징 (v6/v7 물리 수정 이전 버전):
#   - 통합 채널 생산 (Unified ChannelProducer, 단일 TF 프로세스)
#   - IPC V7 + futex, UL fused RawKernel, CUDA Graph
#   - attach-stable / p1b-distance-mode / default-distance-m 미지원
#   - 본 스크립트는 launch_all_v7.sh에서 v4 미지원 파라미터를 제거한 baseline
#
# 사용법:
#   sudo bash launch_all_v4.sh [옵션]
#
# 옵션:
#   -v VERSION   Proxy 버전                         (기본: v4)
...
#   -h           도움말
#
# v4에서 제거된 옵션 (v6/v7 전용, v4.py 미지원):
#   -dist, -stable, -stable-mode, -stable-trigger, -stable-delay, -p1bdist
#
# 예시:
#   sudo bash launch_all_v4.sh                             # v4, gpu-ipc, 1 UE, SISO
#   sudo bash launch_all_v4.sh -n 1 -ga 2 1 -ua 2 1       # v4, 2x1 MIMO
#   sudo bash launch_all_v4.sh -snr 10                     # SNR=10dB
#
# 로그 파일:
#   ~/DevChannelProxyJIN/logs/YYYYMMDD_HHMMSS_G1C_v4_ipc_1ue_ga2x1_ua2x1/
...
# ── 인자 파싱 ─────────────────────────────────────────────────────
usage() {
    echo "G1C 통합 런처 — v4 Channel Proxy (Unified ChannelProducer baseline)"
    echo ""
    echo "사용법: sudo bash launch_all_v4.sh [옵션]"
    echo ""
    echo "옵션:"
    echo "  -v VERSION   Proxy 버전                  (기본: v4)"
    ...
    echo "  -speed M/S   UE 이동 속도 (v4는 내장 default)"
    echo "  -gpu IDX     사용할 GPU 인덱스 (기본: 0)"
    echo "  --xla        XLA JIT 컴파일 활성화 (커널 퓨전 최적화)"
    echo "  -gse N       GT save every N UL slots (기본: 10, env GT_SAVE_EVERY 으로 override 가능)"
    echo "  -mf N        N 프레임 수집 후 자동 종료 (GT seq 기준, 100 단위 반올림)"
    echo "  -h           도움말"
    echo ""
    echo "v4에서 제거된 옵션 (v6/v7 전용):"
    echo "  -dist, -stable, -stable-mode, -stable-trigger, -stable-delay, -p1bdist"
    echo ""
    echo "버전 설명:"
    echo "  v4  Unified ChannelProducer baseline (v6/v7 물리 수정 이전)"
    echo ""
    echo "예시:"
...
echo "    GT    : every ${GT_SAVE_EVERY} UL slot(s), symbols=${GT_SYMBOLS}"
echo "    로그  : ${LOG_DIR}/"
echo "    GT 디렉토리: ${GT_DIR}"
if [ -n "$DURATION" ]; then
