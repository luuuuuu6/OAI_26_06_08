#!/bin/bash
#
# Q4 SNR Sweep Runner — v8 Channel Proxy Edition
# ================================================
# Uses v8.py (v7 + async GT D2H pipeline; safe with --gt-save-every=1).
#
...
LAUNCH_SCRIPT="${SCRIPT_DIR}/launch_all_v8.sh"
...
for SNR in $SNR_POINTS; do
    ...
    if [ -d "$TARGET_DIR/sionna_gt" ]; then
        ...
        if [ "$EXIST_SEQS" -ge "$REQ_SEQS" ] \
           ...
           && [ "$EXIST_SRS_FRAMES" -ge "$MIN_SRS_FRAMES" ]; then
            echo ""
            echo "[sweep] SNR=${SNR} dB already complete (gt=${EXIST_SEQS} srs_bins=${EXIST_SRS} srs_frames=${EXIST_SRS_FRAMES}) in ${TARGET_DIR} — skipping"
            printf "%-10s  %-10s  %-8s  %-8s  %s\n" "$SNR" "SKIP-EXIST" "$EXIST_SEQS" "$EXIST_SRS" \
                "$(basename "$TARGET_DIR")" >> "$MANIFEST"
            continue
        fi
        echo "[sweep] SNR=${SNR} dB has partial data (gt=${EXIST_SEQS} srs_bins=${EXIST_SRS} srs_frames=${EXIST_SRS_FRAMES}) — removing and re-running"
        rm -rf "$TARGET_DIR"
    fi
    ...
    echo "[sweep] SNR=${SNR} dB  status=${STATUS}  gt=${N_SEQS}/${REQ_SEQS}  srs_bins=${N_SRS}/${MIN_SRS_BINS}  srs_frames=${N_SRS_FRAMES}/${MIN_SRS_FRAMES}  dur=${DUR}s"
