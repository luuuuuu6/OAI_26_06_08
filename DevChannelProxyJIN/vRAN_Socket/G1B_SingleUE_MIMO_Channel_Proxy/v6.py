"""
================================================================================
v6.py - GPU IPC V6 + MIMO CUDA Graph Pipeline + Dual Noise Model (G1B)

[v5 → v6 변경사항]
  1. 절대 noise 모드 추가 (--noise-dBFS): 고정 noise floor, PL과 결합 가능
  2. 기존 상대 SNR 모드 (--snr-dB) 유지, 두 모드 상호 배타적
  3. launch_all.sh: -nf (noise-dBFS), -d (duration) 옵션 추가
...
def main():
    ...
    if args.enable_gpu and GPU_AVAILABLE:
        ...
        if args.mode == "gpu-ipc":
            print(f"  + [v6] GPU IPC V6 per-buffer antenna (gNB={args.gnb_ant}, UE={args.ue_ant})")
            print(f"  + [v6] gNB array: {args.gnb_ny}x{args.gnb_nx}, UE array: {args.ue_ny}x{args.ue_nx}")
            print("  + [v6] DL: broadcast*sum channel (CUDA Graph capturable)")
            print("  + [v6] UL: H^T broadcast*sum (DL/UL symmetric)")
            print("  + [v6] OFDM symbol: GPU index array (no Python for-loop)")
            print("  + [v6] Pre-allocated intermediate buffers (out= params)")
            print(f"  + [v6] Noise model: {noise_mode} " +
