"""
================================================================================
v5.py - GPU IPC V6 + MIMO CUDA Graph Pipeline (G1B)

[v4 → v5 변경사항]
  1. einsum → broadcast multiply + sum (cuBLAS 제거, CUDA Graph 호환)
  2. OFDM symbol extraction/reconstruction: Python for-loop → GPU 인덱스 배열
  3. cp.zeros() 동적 할당 → __init__ 사전 할당 버퍼 (out= 파라미터)
  4. 모든 GPU 연산이 CUDA Graph capture에 안전
...
class GPUSlotPipeline:
    ...
    def __init__(self, fft_size=2048, enable_gpu=True, use_pinned_memory=True,
        ...
        print(f"[GPU Pipeline v5] MIMO CUDA Graph Pipeline initializing (n_tx={n_tx_in}, n_rx={n_rx_out})...")
        print(f"[GPU Pipeline v5] Precision: complex128 (float64, PSS stability)")
        print(f"[GPU Pipeline v5] Profiling: interval={self.profile_interval}, "
        ...
        print(f"[GPU Pipeline v5] Initialization complete (n_tx={self.n_tx}, n_rx={self.n_rx})")
    ...
    def _gpu_compute_core(self, pl_linear, snr_db, noise_on):
        ...
        # 3. FFT + broadcast multiply + sum (replaces einsum, no cuBLAS)
        self._tmp_Xf = cp.fft.fft(self.gpu_x, axis=-1)
        self._tmp_Hf = cp.fft.fft(self.gpu_H, axis=-1)
        cp.multiply(self._tmp_Hf, self._tmp_Xf[:, cp.newaxis, :, :], out=self._buf_HX)
...
def main():
    ap = argparse.ArgumentParser(
        description="vRAN Sionna Channel Simulator - v5 (GPU IPC V6 + MIMO CUDA Graph Pipeline)")
    ...
    print("=" * 80)
    print("vRAN Sionna Channel Simulator - v5: GPU IPC V6 + MIMO CUDA Graph Pipeline")
    print("=" * 80)
    ...
    if args.enable_gpu and GPU_AVAILABLE:
        ...
        if args.mode == "gpu-ipc":
            print(f"  + [v5] GPU IPC V6 per-buffer antenna (gNB={args.gnb_ant}, UE={args.ue_ant})")
            print(f"  + [v5] gNB array: {args.gnb_ny}x{args.gnb_nx}, UE array: {args.ue_ny}x{args.ue_nx}")
            print("  + [v5] DL: broadcast*sum channel (CUDA Graph capturable)")
            print("  + [v5] UL: H^T broadcast*sum (DL/UL symmetric)")
            print("  + [v5] OFDM symbol: GPU index array (no Python for-loop)")
            print("  + [v5] Pre-allocated intermediate buffers (out= params)")
            if args.gnb_ant != args.ue_ant:
                print(f"  + [v5] Asymmetric MIMO: bypass=truncate/zero-pad, channel=broadcast*sum")
