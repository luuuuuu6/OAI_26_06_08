"""
================================================================================
v1.py - GPU IPC V5 Circular Buffer Pipeline (G1B)

[v0 → v1 변경사항]
  1. GPUIpcV5Interface: Circular buffer + timestamp indexing (V1의 ready-flag 대신)
  2. DL/UL 대칭 채널: custom_channel=True → DL, UL 모두 Sionna 채널 적용
  3. -b (bypass) → DL, UL 모두 패스스루
  4. Socket mode 1:1 대응: gap zero-fill, initial sync, circular buffer
...
GPU_IPC_V5_SHM_PATH = "/tmp/oai_gpu_ipc/gpu_ipc_shm"
GPU_IPC_V5_MAGIC = 0x47505536
GPU_IPC_V5_VERSION = 7
GPU_IPC_V5_HANDLE_SIZE = 64
GPU_IPC_V5_SHM_SIZE = 4096
GPU_IPC_V5_CIR_SIZE = 460800
GPU_IPC_V5_SAMPLE_SIZE = 4
GPU_IPC_V5_BUF_BYTES = GPU_IPC_V5_CIR_SIZE * GPU_IPC_V5_SAMPLE_SIZE

GPU_IPC_SHM_PATH = GPU_IPC_V5_SHM_PATH


class GPUIpcV5Interface:
    """
    GPU IPC V5 Circular Buffer interface — SERVER role (Proxy).

    Allocates 4 GPU circular buffers, exports IPC handles, creates SHM file.
    gNB and UE are CLIENTs that open the handles via cudaIpcOpenMemHandle.

    4 circular GPU buffers (each CIR_SIZE * 4 bytes):
      dl_tx: gNB writes DL at ts offset → Proxy reads
      dl_rx: Proxy writes processed DL at ts offset → UE reads
      ul_tx: UE writes UL at ts offset → Proxy reads
      ul_rx: Proxy writes processed UL at ts offset → gNB reads
    """

    def __init__(self, shm_path=GPU_IPC_V5_SHM_PATH):
        ...
        self._gpu_mem = []
        self.cir_size = GPU_IPC_V5_CIR_SIZE
        self.buf_bytes = GPU_IPC_V5_BUF_BYTES
        self.initialized = False
        self._last_dl_tx_ts = 0
        self._last_ul_tx_ts = 0

    def init(self):
        """Allocate 4 circular GPU buffers, export IPC handles, create SHM."""
        ...
        os.ftruncate(self.shm_fd, GPU_IPC_V5_SHM_SIZE)
        self.shm_mm = mmap.mmap(self.shm_fd, GPU_IPC_V5_SHM_SIZE)

        self.shm_mm[:] = b'\x00' * GPU_IPC_V5_SHM_SIZE
        ...
        for name, offset in zip(handle_names, handle_offsets):
            mem = cp.cuda.alloc(self.buf_bytes)
            self._gpu_mem.append(mem)
            ptr = mem.ptr
            ptrs.append(ptr)

            cp.cuda.runtime.memset(ptr, 0, self.buf_bytes)

            handle_bytes = cp.cuda.runtime.ipcGetMemHandle(ptr)
            self.shm_mm[offset:offset + GPU_IPC_V5_HANDLE_SIZE] = handle_bytes
            print(f"[GPU IPC V5] SERVER: allocated {name} "
                  f"({self.buf_bytes} bytes, cir={self.cir_size}, ptr=0x{ptr:x})")
        ...
        struct.pack_into('<I', self.shm_mm, 264, self.cir_size)
        struct.pack_into('<I', self.shm_mm, 268, 1)
        struct.pack_into('<I', self.shm_mm, 260, GPU_IPC_V5_VERSION)

        self.shm_mm.flush()
        struct.pack_into('<I', self.shm_mm, 256, GPU_IPC_V5_MAGIC)
        self.shm_mm.flush()

        self.initialized = True
        print(f"[GPU IPC V5] SERVER: ready (magic=0x{GPU_IPC_V5_MAGIC:08X}, "
              f"version={GPU_IPC_V5_VERSION}, cir_size={self.cir_size})")
        return True

    def circ_offset(self, ts, nbAnt):
        """Compute circular buffer offset in sample slots."""
        return int((ts * nbAnt) % self.cir_size)

    def get_gpu_array_at(self, base_ptr, ts, nsamps, nbAnt, dtype=cp.int16):
        """Get a CuPy view into the circular buffer at timestamp position.
        Returns (array, wraps) — wraps=True if data wraps around."""
        off = self.circ_offset(ts, nbAnt)
        total = nsamps * nbAnt
        elem_size = dtype().itemsize
        if off + total <= self.cir_size:
            byte_off = off * GPU_IPC_V5_SAMPLE_SIZE
            n_elem = (total * GPU_IPC_V5_SAMPLE_SIZE) // elem_size
            mem = cp.cuda.UnownedMemory(base_ptr + byte_off,
                                        total * GPU_IPC_V5_SAMPLE_SIZE, owner=None)
            return cp.ndarray(n_elem, dtype=dtype, memptr=cp.cuda.MemoryPointer(mem, 0)), False
        else:
            return None, True

    def gpu_circ_copy(self, dst_ptr, src_ptr, ts, nsamps, nbAnt):
        """GPU-to-GPU circular copy (handles wrap-around)."""
        off = self.circ_offset(ts, nbAnt)
        total = nsamps * nbAnt
        sample_sz = GPU_IPC_V5_SAMPLE_SIZE
    ...
    def get_flat_gpu_array(self, base_ptr, nbytes, dtype=cp.int16):
        """Get flat CuPy array over entire buffer (for channel processing)."""
        n_elements = nbytes // dtype().itemsize
        mem = cp.cuda.UnownedMemory(base_ptr, nbytes, owner=None)
        return cp.ndarray(n_elements, dtype=dtype,
                          memptr=cp.cuda.MemoryPointer(mem, 0))
    ...
    def get_last_dl_tx_ts(self):
        return self.read_shm_field(272, '<Q')

    def get_last_dl_tx_nsamps(self):
        return self.read_shm_field(280, '<I')

    def get_last_dl_tx_nbAnt(self):
        return self.read_shm_field(284, '<I')

    def set_last_dl_rx_ts(self, ts):
        self.write_shm_field(288, '<Q', ts)

    def get_last_ul_tx_ts(self):
        return self.read_shm_field(296, '<Q')

    def get_last_ul_tx_nsamps(self):
        return self.read_shm_field(304, '<I')

    def get_last_ul_tx_nbAnt(self):
        return self.read_shm_field(308, '<I')

    def set_last_ul_rx_ts(self, ts):
        self.write_shm_field(312, '<Q', ts)
...
class Proxy:
    ...
    def _ipc_apply_channel(self, src_ptr, dst_ptr, ts, nsamps, nbAnt):
        """Apply Sionna channel to one full slot at ts. Falls back to passthrough on wrap."""
        arr_in, wraps = self.ipc.get_gpu_array_at(src_ptr, ts, nsamps, nbAnt, cp.int16)
        if wraps:
            self.ipc.gpu_circ_copy(dst_ptr, src_ptr, ts, nsamps, nbAnt)
            return

        arr_out, _ = self.ipc.get_gpu_array_at(dst_ptr, ts, nsamps, nbAnt, cp.int16)
        n_sym = N_SYM
        channels = self.channel_buffer.get_batch(n_sym)
    ...
    def _ipc_process_range(self, src_ptr, dst_ptr, set_ts_fn,
                           start_ts, delta, nbAnt, direction):
        ...
        Remaining partial data gets passthrough copy.
        ...
        while remaining >= slot_samples and apply_ch:
            self._ipc_apply_channel(src_ptr, dst_ptr, pos, slot_samples, nbAnt)
            pos += slot_samples
            remaining -= slot_samples
            slots += 1

        if remaining > 0:
            self.ipc.gpu_circ_copy(dst_ptr, src_ptr, pos, remaining, nbAnt)
            pos += remaining
    ...
    def run_ipc(self):
        """Main loop for GPU IPC V5 — range-based copy on circular buffers.
        ...
        """
        self.ipc = GPUIpcV5Interface(self.ipc_shm_path)
        if not self.ipc.init():
            print("[ERROR] GPU IPC V5 initialization failed")
            return

        self._warmup_pipeline()
        print("[GPU IPC V5] Entering main loop (range-based copy)...")
        dl_count = 0
        ul_count = 0
        t_start = time.time()
        proxy_dl_head = 0
        ...
        try:
            while True:
                ...
                dl_nba = self.ipc.get_last_dl_tx_nbAnt()
                if cur_dl_ts > 0 and dl_nsamps > 0 and dl_nba > 0:
                    gnb_dl_head = cur_dl_ts + dl_nsamps
                    if gnb_dl_head > proxy_dl_head:
                        if proxy_dl_head == 0:
                            proxy_dl_head = max(0, gnb_dl_head - self.ipc.cir_size)
                        dl_nbAnt = dl_nba
                        ...
                        dl_ms, n_slots = self._ipc_process_range(
                            ...
                            proxy_dl_head, delta, dl_nbAnt, "DL")
                ...
                if cur_ul_ts > 0 and ul_nsamps > 0 and ul_nba > 0:
                    ...
                    if ue_ul_head > proxy_ul_head:
                        ...
                        ul_ms, n_slots = self._ipc_process_range(
                            ...
                            proxy_ul_head, delta, ul_nbAnt, "UL")
            ...
            print(f"\n[GPU IPC V5] Terminated by Ctrl-C (DL: {dl_count}, UL: {ul_count})")
        finally:
            self.ipc.cleanup()
...
def main():
    ap = argparse.ArgumentParser(
        description="vRAN Sionna Channel Simulator - v1 (GPU IPC V5 Circular Buffer)")
    ...
    print("=" * 80)
    print("vRAN Sionna Channel Simulator - v1: GPU IPC V5 Circular Buffer + CUDA Graph")
    print("=" * 80)
    ...
    if args.enable_gpu and GPU_AVAILABLE:
        ...
        if args.mode == "gpu-ipc":
            print("  + [v1] GPU IPC V5 circular buffer (timestamp-indexed)")
            print("  + [v1] DL: GPU->GPU channel processing (circular buffer)")
            print("  + [v1] UL: GPU->GPU channel processing (DL/UL symmetric)")
