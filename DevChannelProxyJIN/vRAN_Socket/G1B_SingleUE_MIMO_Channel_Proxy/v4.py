"""
================================================================================
v4.py - GPU IPC V6 + MIMO Channel Application (G1B)

[v3 → v4 변경사항]
  1. Sionna MIMO 채널 생성: PanelArray(Nx,Ny) 기반 다중 안테나
  2. GPUSlotPipeline: per-direction pipeline (DL/UL), einsum 기반 채널 적용
  3. ChannelProducer: tf.squeeze 제거 → 명시적 축 제거, (N_sym, N_r, N_t, FFT)
  4. _ipc_apply_channel: DL/UL 방향별 H / H^T 자동 전환
  5. SISO(1x1)는 MIMO 특수 경우로 동일 코드 경로

[아키텍처]
  gNB(H2D@ts) → [gpu_dl_tx circ] → Proxy(DL: H@X) → [gpu_dl_rx circ] → UE(D2H@ts)
  UE(H2D@ts) → [gpu_ul_tx circ] → Proxy(UL: H^T@X) → [gpu_ul_rx circ] → gNB(D2H@ts)

[핵심 특징]
- GPU IPC V6: per-buffer antenna config (비대칭 MIMO)
- DL/UL 대칭 채널 처리 (-b 플래그로 동시 패스스루)
- 채널 생성: TensorFlow/Sionna (GPU), MIMO H(N_r, N_t, FFT)
- 신호 처리: CuPy einsum (CUDA) - CUDA Graph
...
class GPUSlotPipeline:
    ...
    def __init__(self, fft_size=2048, enable_gpu=True, use_pinned_memory=True,
        ...
        print(f"[GPU Pipeline v4] MIMO Channel Pipeline initializing (n_tx={n_tx_in}, n_rx={n_rx_out})...")
        print(f"[GPU Pipeline v4] Precision: complex128 (float64, PSS stability)")
        print(f"[GPU Pipeline v4] Profiling: interval={self.profile_interval}, "
        ...
        print(f"[GPU Pipeline v4] Initialization complete (n_tx={self.n_tx}, n_rx={self.n_rx})")
    ...
    def _gpu_compute_core(self, pl_linear, snr_db, noise_on):
        """MIMO channel application: de-interleave → einsum → re-interleave.
        Supports SISO(1x1) as special case of MIMO."""
        n_tx = self.n_tx
        n_rx = self.n_rx
        ...
        # 2. OFDM symbol extraction per antenna: (n_sym, n_tx, fft_size)
        for i, (s, e) in enumerate(self.sym_bounds):
            cp_l = CP1 if i < 12 else CP2
            self.gpu_x[i] = self._tmp_cpx_2d[s + cp_l:e, :].T

        # 3. FFT + einsum channel application
        self._tmp_Xf = cp.fft.fft(self.gpu_x, axis=-1)
        self._tmp_Hf = cp.fft.fft(self.gpu_H, axis=-1)
        self._tmp_Yf = cp.einsum('srtf,stf->srf', self._tmp_Hf, self._tmp_Xf)
        self._tmp_y = cp.fft.ifft(self._tmp_Yf, axis=-1)

        # 4. Reconstruct OFDM: scatter + CP insert → (total_cpx, n_rx)
        self._tmp_out_2d = cp.zeros((self.total_cpx, n_rx), dtype=cp.complex128)
        for i, (s, e) in enumerate(self.sym_bounds):
            cp_l = CP1 if i < 12 else CP2
            self._tmp_out_2d[s + cp_l:e, :] = self._tmp_y[i].T
            self._tmp_out_2d[s:s + cp_l, :] = self._tmp_y[i, :, -cp_l:].T

        # 5. Path Loss + AWGN
        self.gpu_out[:] = self._tmp_out_2d.ravel()
        if pl_linear != 1.0:
        ...
        # 6. Re-interleave: (total_cpx, n_rx) → flat int16
        self._tmp_out_2d_final = self.gpu_out.reshape(self.total_cpx, n_rx)
        self._tmp_iq_out_3d = cp.zeros((self.total_cpx, n_rx, 2), dtype=cp.float64)
        self._tmp_iq_out_3d[:, :, 0] = cp.clip(cp.around(self._tmp_out_2d_final.real), -32768, 32767)
        self._tmp_iq_out_3d[:, :, 1] = cp.clip(cp.around(self._tmp_out_2d_final.imag), -32768, 32767)
        self.gpu_iq_out[:] = self._tmp_iq_out_3d.ravel().astype(cp.int16)
    ...
    def process_slot(self, iq_bytes, channels_gpu, pl_linear, snr_db, noise_on):
        ...
        with self.stream:
            ...
            if ch_slice.dtype != cp.complex128:
                self.gpu_H[:n_ch, :ch_slice.shape[1], :ch_slice.shape[2], :n_w] = ch_slice.astype(cp.complex128)
            else:
...
class ChannelProducer(threading.Thread):
    ...
    def run(self):
        ...
        while not self.stop_event.is_set():
            ...
            # h_cp_batch: (N_sym, N_r, N_t, N_FFT) — first dim is batch for RingBuffer
...
class Proxy:
    ...
    def _init_channel(self, buffer_len, buffer_symbol_size):
        ...
        # RingBuffer: each entry = (ue_ant, gnb_ant, FFT_SIZE) = MIMO H matrix
        ...
        self.pipeline_ul = GPUSlotPipeline(
            FFT_SIZE, n_tx_in=self.ue_ant, n_rx_out=self.gnb_ant, **pipeline_common)
        self.gpu_slot_pipeline = self.pipeline_dl
        print(f"[INFO] GPU Slot Pipeline v4 (MIMO) initialized "
    ...
    def _ipc_apply_channel(self, src_ptr, dst_ptr, ts, nsamps,
        ...
        channels = self.channel_buffer.get_batch(N_SYM)
        # channels: (N_SYM, ue_ant, gnb_ant, FFT_SIZE) from RingBuffer
    ...
    def _warmup_pipeline(self):
        ...
        if not self.pipeline_dl or not self.pipeline_dl.enable_gpu:
            return
        if not self.custom_channel:
            print("[GPU IPC V6] Bypass mode — channel warmup skipped")
            return
        print("[GPU IPC V6] Pre-warming DL/UL pipelines (XLA compile + CUDA Graph)...")
        t0 = time.time()
        ...
        for label, pipeline in [("DL", self.pipeline_dl), ("UL", self.pipeline_ul)]:
            ...
            for i in range(passes):
                pipeline.process_slot_ipc(
                    dummy_in, dummy_ch, pathLossLinear, snr_dB, noise_enabled, dummy_out
                )
                print(f"  {label} warmup {i+1}/{passes} done ({time.time()-t0:.1f}s)")
        print(f"[GPU IPC V6] DL/UL pipelines ready ({time.time()-t0:.1f}s)")
...
def main():
    ap = argparse.ArgumentParser(
        description="vRAN Sionna Channel Simulator - v4 (GPU IPC V6 + MIMO Channel Application)")
    ...
    print("=" * 80)
    print("vRAN Sionna Channel Simulator - v4: GPU IPC V6 + MIMO Channel Application")
    print("=" * 80)
    ...
    if args.enable_gpu and GPU_AVAILABLE:
        ...
        if args.mode == "gpu-ipc":
            print(f"  + [v4] GPU IPC V6 per-buffer antenna (gNB={args.gnb_ant}, UE={args.ue_ant})")
            print(f"  + [v4] gNB array: {args.gnb_ny}x{args.gnb_nx}, UE array: {args.ue_ny}x{args.ue_nx}")
            print("  + [v4] DL: H@X einsum channel (circular buffer)")
            print("  + [v4] UL: H^T@X einsum channel (DL/UL symmetric)")
            if args.gnb_ant != args.ue_ant:
                print(f"  + [v4] Asymmetric MIMO: bypass=truncate/zero-pad, channel=einsum")
